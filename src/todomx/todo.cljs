...
(defn todo-list []
  (md/make ::todo-list
    ...
    :items (c? (doall (remove td-deleted (md-get me :items-raw))))
    ...
    :items-completed (c? (doall (filter td-completed (md-get me :items))))
    :items-active (c? (doall (remove td-completed (md-get me :items))))
